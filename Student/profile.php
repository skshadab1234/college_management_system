   ...
   <body>
      <div class="main-content">
         ...
         <nav  class="navbar navbar-top navbar-expand-md navbar-dark" id="navbar-main">
            <ul class="navbar-nav align-items-center d-none d-md-flex" style="position: absolute;right: 20px;top: 10px">
          <li class="nav-item dropdown">
            <a class="nav-link pr-0" href="<?= FRONT_SITE_PATH.'logout'.PHP_EXT?>" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
...
?>
    ...
    <script type="text/javascript" src="https://cdnjs.cloudflare.com/ajax/libs/limonte-sweetalert2/10.16.0/sweetalert2.all.min.js"></script>
      ...
      <script>
         $(document).ready(()=>{
            $('input').not('#input-fileup').attr('disabled',true);

             $("#submit_grevieance").click( () =>{
               var description  = $("#input-description").val();
               var full_name_student = $("#full_name_student").val();
               
               jQuery('#submit_grevieance').attr('disabled',true);
               jQuery('#submit_grevieance').html("Wait.....");
                  ...
                  if (description.length >= 30) {
                    $.ajax({
                        ...
                        success:function(data) {
                           ...
                           if (result.status == 'success') {
                               ...
                               Swal.fire({
                                   position: 'top-end',
                                   icon: 'success',
                                   title: 'Grievenance Sended',
                                    text: 'Dear '+full_name_student+', Thanks to let us know that some mistake happen in your profile. We will try to fix your problem today itself only',
                                   showConfirmButton: false,
                                   timer: 3500
                                 })
                               $("#input-description").val('');
                               jQuery('#submit_grevieance').attr('disabled',false);
                               jQuery('#submit_grevieance').html("Send");
                            }
                        }
                    })
                  }else{
                     Swal.fire({
                       icon: 'error',
                       title: 'Oops...',
                       text: 'Message Should be >= 30 Character',
                     })
                     jQuery('#submit_grevieance').attr('disabled',false);
                     jQuery('#submit_grevieance').html("Send");
                  }
